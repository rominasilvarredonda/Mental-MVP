...
export function OnboardingWizard({ step, total, question, answers, error, isComplete, onClose, onSaveAnswer, onSelectAnswer, onPrevious, onNext, onFinish }: OnboardingWizardProps) {
  ...
  return <ScreenLayout progress={((step + 1) / total) * 100} onClose={onClose}><section className={styles.wizard}>
    ...
    <div className={styles.card}><p className={styles.cardLabel}>{question.multiple ? "SELECCIÓN MÚLTIPLE" : "ELEGÍ UNA OPCIÓN"}</p><h1>{question.title}</h1>{question.helper && <p className={styles.helper}>{question.helper}</p>}{question.input ? <Input className={styles.ageInput} type="number" inputMode="numeric" min="1" value={String(answers[question.id] ?? "")} onChange={(event) => onSaveAnswer(question.id, event.target.value)} placeholder="Ingresá tu edad" /> : <Options question={question} answers={answers} onSaveAnswer={onSaveAnswer} onSelectAnswer={onSelectAnswer} />}</div>
    <p className={styles.error} role="alert">{error}</p><footer className={styles.actions}><button type="button" className={styles.back} onClick={step ? onPrevious : onClose}>← Volver</button><Button type="button" onClick={onNext}>{step === total - 1 ? "Finalizar" : "Continuar"} →</Button></footer>
